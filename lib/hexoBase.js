...
class HexoBase {
  ...
  async copyFramework(docPath, targetDir) {
    ...
    // $baseDir/docs/source/languages > $targetDir/themes/pandora/languages
    ...
    if (await fs.exists(languagePath)) {
      ...
      await cpy('*.yml', path.join(targetDir, 'themes/pandora/languages'), { cwd: languagePath });
    }

    // $baseDir/docs/theme/* > $targetDir/themes/pandora
    const themePath = path.join(docPath, 'theme');
    await cpy('**/*', path.join(targetDir, 'themes/pandora'), { cwd: themePath, nodir: true, parents: true });
  }
